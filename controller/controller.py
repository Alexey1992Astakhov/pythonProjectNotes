...
def run():
    ...
    while True:
        value = input(':>>> ')
        arg = check_input_args(value)  # проверка аргумента переданного из консоли

        if arg == 'add':
            while True:
                BaseUI.add_note_ui()  # функция добавления заметки в файл
                print(draw_bord(42))
            ...
            while True:
                ...
                if arg == 'go':
                    while True:
                        BaseUI.show_note_ui()
                        print(draw_bord(42))
                        print('> Просмотр еще одной заметки, введите "1" <\n> для выхода в главное меню введите любое значение <')
                        arg = input(':>>> ')
                        if arg == "1":
                            continue
                        else:
