...
class MainUI:
    ...
    def help_ui():
        '''метод показывет список команд для работы с приложением'''
...
class AddUI:
    ...
    def add_note_ui():
        ...
        n_h.addNote(note)  # добавляем заметку в файл
...
class ShowUI:
    ...
    def sorted_by_date(value=False):
        ...
        print(draw_bord(42))
        [print('{:2s} || {:10s}'.format(note.title, note.time)) for note in notes_list]
        print(draw_bord(42))

    def filter_by_dates():
        '''метод, выборки данных по указанной дате(-ам)'''
        while True:
            console_clear()

            print("Введите даты в формате: 01 01 2001")
            date1 = input('первая дата :>>> ')
            while not check_date(date1):
                print("некорректное значение, повторите ввод.")
                print("первая дата является обязательным параметром")
                print("Введите дату в формате: 01 01 2001")
                date1 = input(':>>> ')
